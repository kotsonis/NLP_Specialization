...
#    print('shape of theta: ' + str(theta.shape))
    for i in range(0, num_iters):
        ...
        z = np.dot(x,theta)
#        if i == 0 : print('shape of z: ' +str(z.shape))
        # get the sigmoid of z
        h = sigmoid(z)
#        if i == 0 : print('shape of h: ' +str(h.shape))
        # calculate the cost function
        J = -np.sum(np.dot(np.transpose(y),np.log(h))+np.dot(np.transpose(1-y),np.log(1-h)))/(float(m))
        # update the weights theta
#        print('shape of x: ' +str(x.shape))
#        print('shape of h: ' + str(h.shape))
#        print('shape of y: ' + str(y.shape))
        dJ = np.sum(np.dot(np.transpose(x),(h-y)),axis=1, keepdims=True)/(float(m))
#        if i == 0 : print('shape of dJ: ' + str(dJ.shape))
#        print('shape of theta: ' + str(theta.shape))
...
def test_logistic_regression(test_x, test_y, freqs, theta):
    ...
    accuracy = np.sum(np.asarray(y_hat) == np.squeeze(test_y))/(float(test_y.shape[0]))
